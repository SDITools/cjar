...
14. Click on "Service Account (JWT)" under "CREDENTIALS" in the left column. Locate the "Download JSON" button on the top right and click it to download the service account JSON file. Alternatively, you can manually create this file by copying and pasting the Client ID, Client Secret (click “Retrieve client secret”), Technical Account ID, and Organization ID into a `.json` file.  Reference `?cja_auth` for more inforamtion on the variables needed. Using the preconfigured JSON file is the easiest method.
...
After adding all 2 variables to the `.Renviron` file and saving it, restart your R session (**Session > Restart R** in RStudio) and reload `cjar` (`library(cjar)`). 
