...
#' @param metric This is the subject of the rule. The value should be the metric
#' id.  Only the dimension or metric can be used at a time.
#' @param verb Choose from any of the 30 different verbs. Use the [filter_verbs()]
...
#' @param is_distinct This will filter on a distinct count of items within a dimension.
...
#' @param attribution_context When applying a non-repeating instance attribution model
...
#'
#' @return A structured list defining the rule for a filter
#'
#' @import dplyr
#' @import assertthat
#' @import stringr
...
#' @export
filter_rule <- function(dimension = NULL,
                        metric = NULL,
                        verb = NULL,
                        object = NULL,
                        description = NULL,
  ...
  if(validate) {
    ...
    ##metrics
    get_mets <- cja_get_metrics
    if (!memoise::is.memoised(get_mets)) {
      get_mets <- memoise::memoise(get_mets)
    }
    cja_metrics <- get_mets(dataviewId = dataviewId)

    get_dims <- cja_get_dimensions
    if (!memoise::is.memoised(get_dims)) {
      get_dims <- memoise::memoise(get_dims)
    }
    cja_dimensions <- get_dims(dataviewId = dataviewId)

    #define the variable to be either a metric or dimension and save it as the adjective
    adj <- dplyr::case_when(subject %in% cja_metrics$id ~ 'metrics/',
                            subject %in% cja_dimensions$id ~ 'variables/')
  ...
  verbs <- read.csv(system.file("extdata", "filter_verbs.csv", package = "cjar"))
