...
filter_pred <- function(subject = 'page', # one of hits, visits, or visitors
  ...
  ### Define the different elements of a filter
  #########################
  ## Exists Verbs
  ...
  if(!exists('cja_metrics')){
    cja_metrics <- cja_get_metrics(dataviewid = dataid) }
  if(!exists('cja_dimensions')) {
    cja_dimensions <- cja_get_dimensions(dataviewid = dataid)
  }
  ...
  assertthat::assert_that(verb %in% c(exists_verbs, str_verbs, list_verbs, glob_verbs, num_verbs, numlist_verbs),
                          msg = "'verb' argument must be one of the verbs listed on https://www.adobe.io/apis/experiencecloud/analytics/docs.html#!AdobeDocs/analytics-2.0-apis/master/filters.md")
